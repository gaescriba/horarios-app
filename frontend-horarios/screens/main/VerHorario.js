...
export default VerHorario = () => {
  ...
  const idProfesorRoute = route.params?.idProfesor
  ...
  useEffect(() => {
    if(retrievedToken){
      ...
      if(retrievedToken?.idProfesor){
        ...
        console.log('id en route', idProfesorRoute)
        if(idProfesorRoute != 0 ){
          setIdProfesor(idProfesorRoute)
        }
        if(profesor){
          //console.log('hay objeto profesor')
